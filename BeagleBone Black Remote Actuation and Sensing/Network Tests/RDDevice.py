...
def actuateLed(addr, tags, msg, source):
	ledValue = msg[0]
	print "receiving msg: '{0}' '{1}'".format(addr, ledValue)
	if bbbExists == True:
		if ledValue == 1:
...
def senseLdr(addr, tags, msg, source):
	nameOfPlayerToReturnTo = msg[0]
	if bbbExists == True:
		print "polling sensor"
		ldrVal = ADC.read("P9_40")
	else:
		ldrVal = random.uniform(0, 1)
	print "sending msg: '{0}' '{1}' '{2}'".format(pythonSenseReturnOscPath, nameOfPlayerToReturnTo, ldrVal)
	msg = OSC.OSCMessage()
	msg.setAddress(pythonSenseReturnOscPath)
	msg.append(nameOfPlayerToReturnTo)
	msg.append(ldrVal)
