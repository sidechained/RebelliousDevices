...
RDLaptop {
	...
	var sclangSenseOscPath = '/sense/ldr';
	...
	*new {arg argMe, argAddrBook;
	...
	initLdrValueReceiver {
		ldrValueReceiver = OSCFunc({arg msg;
			(me.name.asString + "receiving: " + sclangSenseOscPath.asString + msg).postln;
			// what can we do with this now?!
		}, sclangSenseOscPath, nil, me.addr.port) // doesn't work if srcID bound to me.addr (why not?)
	}
